...
class EarlyStoppingCallback(TrainerCallback):
    ...
    def __init__(self, early_stopping_patience: int = 1, early_stopping_threshold: Optional[float] = 0.0,
                 patience_threshold: Optional[float] = 0.0):
        self.early_stopping_patience = early_stopping_patience
        self.early_stopping_threshold = early_stopping_threshold
        self.patience_threshold = patience_threshold  # only patiance++ if cur is this much worse than best
    ...
    def check_metric_value(self, args, state, control, metric_value):
        ...
        if state.best_metric is None or (
            ...
            self.early_stopping_patience_counter = 0
            logger.warning(
                f"\nnew best:{metric_value} [only meaningful if saving model!]"
            )
        else:
            if abs(metric_value - state.best_metric) > self.patience_threshold:
                self.early_stopping_patience_counter += 1
                logger.warning(
                    f"\ncur:{metric_value} best:{state.best_metric} patience={self.early_stopping_patience_counter}\n"
                )
            else:
                logger.warning(
                    f"\n[patience remains since not significantly worse]"
                    f" cur:{metric_value} best:{state.best_metric} patience={self.early_stopping_patience_counter}\n"
                )
